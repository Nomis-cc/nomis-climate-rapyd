...
namespace Nomis.Rapyd
{
        ...
        ISingletonService
    {
        ...
        {
            var apiResponse = await MakeRequest<RapydResponse<List<RapydCustomer>>>(Method.Get, "/v1/customers");
            var result = apiResponse;
            return await Result<List<RapydCustomer>>.SuccessAsync(result?.Data ?? new List<RapydCustomer>(), "Got a list of all customers.");
        }

        /// <inheritdoc />
        public async Task<Result<RapydCustomer?>> CreateCustomerAsync(RapydCreateCustomerBody body)
        {
            var apiResponse = await MakeRequest<RapydResponse<RapydCustomer>>(Method.Post, "/v1/customers", body);
            var result = apiResponse;
            return await Result<RapydCustomer?>.SuccessAsync(result?.Data, "Customer created successfully.");
        }
        ...
        {
            var apiResponse = await MakeRequest<RapydResponse<RapydCheckout>>(Method.Post, "/v1/checkout", body);
            var result = apiResponse?.Data;
            return await Result<RapydCheckout?>.SuccessAsync(result, "Payment checkout created successfully.");
        }

        /// <inheritdoc />
        public async Task<Result<RapydCheckout?>> GetCheckoutAsync(string checkoutId)
        {
            var apiResponse = await MakeRequest<RapydResponse<RapydCheckout>>(Method.Get, $"/v1/checkout/{checkoutId}");
            var result = apiResponse?.Data;
            return await Result<RapydCheckout?>.SuccessAsync(result, "Got checkout data.");
        }

        /// <inheritdoc />
        public async Task<Result<List<RapydPaymentMethod>>> GetPaymentMethodsAsync(string country)
        {
            var apiResponse = await MakeRequest<RapydResponse<List<RapydPaymentMethod>>>(Method.Get, $"/v1/payment_methods/country?country={country}");
            var result = apiResponse?.Data ?? new List<RapydPaymentMethod>();
            return await Result<List<RapydPaymentMethod>>.SuccessAsync(result, "Got payment methods.");
        }

        /// <inheritdoc />
        public async Task<Result<RapydPaymentMethodRequiredFields?>> GetPaymentMethodRequiredFieldsAsync(string type)
        {
            var apiResponse = await MakeRequest<RapydResponse<RapydPaymentMethodRequiredFields>>(Method.Get, $"/v1/payment_methods/required_fields/{type}");
            var result = apiResponse?.Data;
            return await Result<RapydPaymentMethodRequiredFields?>.SuccessAsync(result, "Got payment method required fields.");
        }

        /// <inheritdoc />
        public async Task<Result<RapydPayment?>> CreatePaymentAsync(RapydCreatePaymentBody body)
        {
            var apiResponse = await MakeRequest<RapydResponse<RapydPayment>>(Method.Post, "/v1/payments", body);
            var result = apiResponse?.Data;
            return await Result<RapydPayment?>.SuccessAsync(result, "Payment created successfully.");
        }

        /// <inheritdoc />
        public async Task<Result<RapydPayment?>> CancelPaymentAsync(string paymentId)
        {
            var apiResponse = await MakeRequest<RapydResponse<RapydPayment>>(Method.Delete, $"/v1/payments/{paymentId}");
            var result = apiResponse?.Data;
            return await Result<RapydPayment?>.SuccessAsync(result, "Payment cancelled successfully.");
        }

        /// <inheritdoc />
        public async Task<Result<List<RapydCustomerPaymentMethod>>> GetCustomerPaymentMethodsAsync(string customerId)
        {
            var apiResponse = await MakeRequest<RapydResponse<List<RapydCustomerPaymentMethod>>>(Method.Get, $"/v1/customers/{customerId}/payment_methods");
            var result = apiResponse?.Data ?? new();
            return await Result<List<RapydCustomerPaymentMethod>>.SuccessAsync(result, "Got customer payment methods.");
        }
        ...
        private string GenerateSign(string method, string urlPath, string salt, long timestamp, string body)
        {
            ...
            {
                bodyString = body == "{}" ? string.Empty : body;
            }
