        ...
        IStatCalculator<PolygonWalletStats, PolygonTransactionIntervalData>
    {
        ...
        public PolygonWalletStats GetStats()
        {
            ...
            var soldSum = IStatCalculator<PolygonWalletStats, PolygonTransactionIntervalData>
                .GetTokensSum(soldTokens.Select(x => x.Hash!), _internalTransactions.Select(x => (x.Hash!, ulong.TryParse(x.Value, out ulong amount) ? amount : new BigInteger(0))));
            ...
            var buySum = IStatCalculator<PolygonWalletStats, PolygonTransactionIntervalData>
                .GetTokensSum(buyTokens.Select(x => x.Hash!), _internalTransactions.Select(x => (x.Hash!, ulong.TryParse(x.Value, out ulong amount) ? amount : new BigInteger(0))));
            ...
            var buyNotSoldSum = IStatCalculator<PolygonWalletStats, PolygonTransactionIntervalData>
                .GetTokensSum(buyNotSoldTokens.Select(x => x.Hash!), _internalTransactions.Select(x => (x.Hash!, ulong.TryParse(x.Value, out ulong amount) ? amount : new BigInteger(0))));
